...
def export_formats():
    """YOLOv8 导出格式。"""
    import pandas  # 导入 pandas 库以便进行数据处理

    # 创建一个包含导出格式及其属性的列表
    x = [
        ["PyTorch", "-", ".pt", True, True],  # PyTorch 格式的详细信息
        ["TorchScript", "torchscript", ".torchscript", True, True],  # TorchScript 格式的详细信息
        ["ONNX", "onnx", ".onnx", True, True],  # ONNX 格式的详细信息
        ["OpenVINO", "openvino", "_openvino_model", True, False],  # OpenVINO 格式的详细信息
        ["TensorRT", "engine", ".engine", False, True],  # TensorRT 格式的详细信息
        ["CoreML", "coreml", ".mlpackage", True, False],  # CoreML 格式的详细信息
        ["TensorFlow SavedModel", "saved_model", "_saved_model", True, True],  # TensorFlow SavedModel 格式的详细信息
        ["TensorFlow GraphDef", "pb", ".pb", True, True],  # TensorFlow GraphDef 格式的详细信息
        ["TensorFlow Lite", "tflite", ".tflite", True, False],  # TensorFlow Lite 格式的详细信息
        ["TensorFlow Edge TPU", "edgetpu", "_edgetpu.tflite", True, False],  # TensorFlow Edge TPU 格式的详细信息
        ["TensorFlow.js", "tfjs", "_web_model", True, False],  # TensorFlow.js 格式的详细信息
        ["PaddlePaddle", "paddle", "_paddle_model", True, True],  # PaddlePaddle 格式的详细信息
        ["NCNN", "ncnn", "_ncnn_model", True, True],  # NCNN 格式的详细信息
    ]

    # 将列表转换为具有指定列名的 DataFrame
